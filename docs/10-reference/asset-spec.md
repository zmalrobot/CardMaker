...
> (`SymbolSet.Key`, `Symbol.Key`, `Template.Key`) descritte in `docs/03-data/data-model.md`.
...
`.otf` **legalmente utilizzabili e incorporabili**, indicando per ognuno la licenza.

> [!IMPORTANT]
> I file `.woff2` **non sono supportati** dal motore di rendering SkiaSharp e vengono rifiutati in upload
> (ADR-017). Convertire in `.ttf` o `.otf` prima della consegna.
...
> Se un ruolo Rush coincide graficamente con quello classico, **non serve un file in più**:
...
(vedi [ADR-010](../09-decisions/adr-001-to-010.md#adr-010)).
