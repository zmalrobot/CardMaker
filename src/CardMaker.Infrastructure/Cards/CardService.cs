    ...
    public async Task<IReadOnlyList<CardTypeDetailDto>> GetGameCardTypesAsync(string gameKey, CancellationToken cancellationToken = default)
    {
        ...
        return cardTypes.Select(ct => new CardTypeDetailDto(
            ...
            ct.Fields.OrderBy(f => f.SortOrder).Select(f => new FieldDefinitionDto(
                ...
                f.SortOrder,
                f.VisibleWhenJson,
                f.OptionList?.Items.OrderBy(i => i.SortOrder).Select(i => new OptionItemDto(i.Key, i.Label.Get("it"))).ToList() ?? [],
                f.SymbolSet?.Symbols.OrderBy(s => s.SortOrder).Select(s => new SymbolOptionDto(s.Key, s.Name.Get("it"), s.AssetId, s.InlineToken)).ToList() ?? [])).ToList(),
            ct.AllowedTraits.Select(t => new TraitOptionDto(t.Trait.Id, t.Trait.Key, t.Trait.Name.Get("it"), t.Trait.Group)).ToList(),
